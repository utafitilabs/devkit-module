...
It is on Packagist, so that line is the whole of it: no repository entry and no
constraint spelled out by hand. `--dev` is not a preference — it is the
production firewall this package lives behind.
